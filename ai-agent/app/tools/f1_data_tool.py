...
def get_driver_standings(year: Union[int, str] = "current") -> Dict[str, Any]:
    """
    Retrieve Driver Championship standings for a given year.
    
    Parameters:
    - year: Season year (e.g. 2021, "current")
    """
    try:
        data = _http_get(f"{year}/driverStandings.json")
        if not data:
            return {"error": f"Failed to retrieve driver standings for {year}.", "year": str(year), "standings": []}

        standings_lists = data.get("MRData", {}).get("StandingsTable", {}).get("StandingsLists", [])
        if not standings_lists:
            return {"error": f"No driver standings data found for {year}.", "year": str(year), "standings": []}
        ...
        return {
            "year": str(curr_list.get("season", year)),
            "round": int(curr_list.get("round", 0)),
...
def get_constructor_standings(year: Union[int, str] = "current") -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not data:
            return {"error": f"Failed to retrieve constructor standings for {year}.", "year": str(year), "standings": []}

        standings_lists = data.get("MRData", {}).get("StandingsTable", {}).get("StandingsLists", [])
        if not standings_lists:
            return {"error": f"No constructor standings data found for {year}.", "year": str(year), "standings": []}

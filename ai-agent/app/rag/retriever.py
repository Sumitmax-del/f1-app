...
def is_knowledge_query(query: str) -> bool:
    """
    Determine if a user question is asking about F1 rules, technical concepts,
    tyre compounds, points system, flags, DRS, ERS, or car dynamics.
    """
    q = query.lower().strip()
    
    # Do NOT treat statistical race result queries as pure RAG
    if any(race_kw in q for race_kw in ["who won", "fastest lap at", "pole position at", "qualifying at", "leading the", "championship standings", "schedule of"]):
        return False
    
    rag_keywords = [
        "drs", "drag reduction",
        "ers", "mgu-k", "mgu-h", "energy recovery", "kinetic", "thermal", "hybrid", "power unit",
        "tyre", "tyres", "tire", "tires", "compound", "pirelli", "slick", "intermediate", "graining", "blistering",
        "point", "points", "scoring", "points system", "how many points",
        "flag", "flags", "yellow flag", "blue flag", "red flag", "black flag", "safety car", "vsc", "virtual safety car",
        "parc ferme", "parc fermé",
        "qualifying format", "sprint format", "knockout", "q1", "q2", "q3",
        "penalty", "penalties", "time penalty", "stop-and-go", "drive-through", "grid drop",
        "apex", "understeer", "oversteer", "downforce", "slipstream", "dirty air", "tow",
        "box box", "pit stop", "what is f1", "how does f1", "explain"
    ]
